 ...
 * <cmd_petrificus>   = 'petrificus' 'totalus' '(' <condicao> ')' '[' <comandos> ']' // while
 * <cmd_protego>      = 'protego' <variavel> 'lumos' <expressao> // atribuicao
 * <cmd_homenum>      = 'homenum' 'revelio' '(' <variavel> ')' // leitura
 * <cmd_expelliarmus> = 'expelliarmus' <variavel> 'avada' 'kedavra' <expressao> // for
 *                      'crucio' <expressao> '[' <comandos> ']' 
 * <cmd_leviosa>      =  'wingardium' 'leviosa' '(' <condicao> ')' '[' <comandos> ']' <else> // if
 * <else>             = epsilon
 *                    | 'alohomora' '[' <comandos> ']'
 * <expressao>        = <expressao> 'reparo' <termo>        // +
 *                    | <expressao> 'dissendium' <termo>    // -
 *                    | <expressao> 'sectumsempra' <termo>  // maior de dois 
 *                    | <termo> 
 * <termo    >        = <termo> 'aguamenti' <fator>         // * 
